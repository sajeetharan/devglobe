...
const mcpEvents = "let M=workspace('c97b958c-a52f-45b5-bcc1-d7eb355850ff').ContainerAppConsoleLogs_CL | where TimeGenerated >= ago(30d) and Log_s has 'devglobe_mcp' | extend Metric=parse_json(Log_s) | where tostring(Metric.event) == 'devglobe_mcp' | extend Timestamp=TimeGenerated, Method=tostring(Metric.method), Tool=iff(isempty(tostring(Metric.tool)), 'none', tostring(Metric.tool)), Resource=iff(isempty(tostring(Metric.resource)), 'none', tostring(Metric.resource)), Client=iff(isempty(tostring(Metric.client)), 'unknown', tostring(Metric.client)), Outcome=tostring(Metric.outcome), DurationMs=todouble(Metric.durationMs), ResultCount=todouble(Metric.resultCount), CallerHash=tostring(Metric.callerHash), ErrorCode=tostring(Metric.errorCode);";
query('mcp-health', 'MCP health - last 30 days', `${mcpEvents} let T=M | summarize Calls=count(), ToolCalls=countif(Method == 'tools/call'), ProjectReads=countif(Method == 'resources/read' and Resource == 'devglobe://project'), CallerDays=dcountif(CallerHash, isnotempty(CallerHash)), Successful=countif(Outcome == 'success'), P95LatencyMs=percentile(DurationMs, 95); T | extend SuccessRate=iff(Calls == 0, 0.0, round(100.0 * Successful / Calls, 1)) | project Metric=pack_array('Requests', 'Tool calls', 'Project resource reads', 'Correlated caller-days', 'Success rate %', 'P95 latency ms'), Value=pack_array(todouble(Calls), todouble(ToolCalls), todouble(ProjectReads), todouble(CallerDays), SuccessRate, P95LatencyMs) | mv-expand Metric to typeof(string), Value to typeof(real)`, 'tiles', null, {
  size: 4,
  tileSettings: {
    titleContent: { columnMatch: 'Metric', formatter: 1 },
    leftContent: { columnMatch: 'Value', formatter: 12, formatOptions: { palette: 'auto' } },
...
query('mcp-daily', 'Daily MCP requests, project reads, and errors', `${mcpEvents} M | summarize Requests=count(), ProjectReads=countif(Method == 'resources/read' and Resource == 'devglobe://project'), Errors=countif(Outcome == 'error') by Day=startofday(Timestamp) | order by Day asc`, 'timechart', '60');
...
query('mcp-clients', 'MCP client mix', `${mcpEvents} M | summarize Requests=count(), ToolCalls=countif(Method == 'tools/call'), ProjectReads=countif(Method == 'resources/read' and Resource == 'devglobe://project') by Client | order by Requests desc`, 'piechart', '50');
