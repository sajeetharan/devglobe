...
import { isAutomatedUserAgent } from './automated-traffic.js';

export { isAutomatedUserAgent };
...
export const ENGAGEMENT_EVENTS = new Set([
  ...
  'profile_claimed',
  'profile_viewed',
  'presence_heartbeat_received',
  'presence_started',
  'presence_stopped',
  'presence_token_issued',
  'personalized_profile_viewed',
  'primary_action_completed',
  'recommendation_opened',
  'repository_match_generated',
  'repository_match_mcp_prompt_copied',
  'repository_match_opened',
  'repository_match_profile_opened',
  'repository_match_shared',
  'repository_match_submitted',
...
const ALLOWED_PROPERTIES = new Set(['action', 'campaign', 'channel', 'journey', 'missionKey', 'responseMinutes', 'source']);
...
const ACQUISITION_SOURCES = new Set([...SHARE_CHANNELS, 'community_event', 'direct', 'discord', 'external_referral', 'github_discussions', 'github_readme', 'local', 'manual_outreach', 'other', 'search', 'search_button', 'search_enter', 'weekly_digest', 'weekly_spotlight']);
const ACQUISITION_CHANNELS = new Set(['community', 'direct', 'email', 'other', 'referral', 'social']);
const ACQUISITION_CAMPAIGNS = new Set(['agents', 'community', 'country_leaderboard', 'developer_activation', 'developer_invite', 'developer_spotlight', 'identity_card', 'india_top_50', 'none', 'organic_referral', 'product', 'rank_movement', 'shared_profile', 'unknown', 'vibe_session', 'weekly_impact']);
...
const AGENT_WORKFLOW_ACTIONS = new Set(['consent-gated-introduction', 'contribution-opportunity-finder', 'repository-contributor-match', 'weekly-developer-scout']);
...
function normalizeProperties(value, eventName) {
  ...
  for (const [key, rawValue] of Object.entries(value)) {
    ...
    let normalized = rawValue.trim().slice(0, key === 'missionKey' ? 43 : 40);
    ...
    if (key === 'action' && eventName === 'agent_workflow_copied' && !AGENT_WORKFLOW_ACTIONS.has(normalized)) normalized = 'unknown';
    if (key === 'responseMinutes' && (!/^\d{1,7}$/.test(normalized) || Number(normalized) < 0)) {
      throw new EngagementValidationError('Invalid responseMinutes');
    }
    if (key === 'missionKey' && !/^[A-Za-z0-9_-]{43}$/.test(normalized)) {
      throw new EngagementValidationError('Invalid missionKey');
    }
...
export function normalizeEngagementEvent(input) {
  ...
  if (['activation_completed', 'card_generated', 'personalized_profile_viewed', 'primary_action_completed', 'profile_claimed', 'profile_primary_action_viewed', 'profile_shared', 'profile_viewed', 'referral_profile_opened', 'search_appearance', 'search_result_opened'].includes(eventName) && !targetLogin) {
...
export function createEngagementEvent(input, options = {}) {
  ...
  const qualifier = normalized.eventName === 'profile_shared'
    ...
    : ['next_action_selected', 'primary_action_completed', 'profile_primary_action_viewed', 'return_briefing_action_selected', 'search_failed', 'search_no_results', 'search_results_viewed', 'search_started'].includes(normalized.eventName)
      ? normalized.properties.action || ''
      : normalized.eventName === 'recommendation_opened'
        ? normalized.properties.journey || ''
        : normalized.eventName === 'mission_maintainer_replied'
          ? normalized.properties.missionKey || ''
        : '';
...
export function aggregateDailyMissionMetrics(events, { now = new Date(), days = 7, threshold = INSIGHTS_PRIVACY_THRESHOLD } = {}) {
  ...
  const replied = sessionsFor('mission_maintainer_replied');
  ...
  const replyEvents = missionEvents.filter(event => event.eventName === 'mission_maintainer_replied');
  const replyCohorts = new Set(replyEvents.map(event => event.privacyHash || event.sessionHash).filter(Boolean));
  const repliesByMission = new Map();
  for (const event of replyEvents) {
    const missionKey = event.properties?.missionKey || event.sessionHash;
    const minutes = Number(event.properties?.responseMinutes);
    if (missionKey && Number.isFinite(minutes) && !repliesByMission.has(missionKey)) repliesByMission.set(missionKey, minutes);
  }
  const uniqueMaintainerReplies = visible(repliesByMission.size);
  const responseMinutes = [...repliesByMission.values()];
  const visibleReturningSessions = visible(returningSessions);
  return {
    days,
    privacyThreshold: threshold,
    suppressed: [uniqueViewers, uniqueAcceptors, uniquePassers, uniqueCompleters, uniqueMaintainerReplies, visibleReturningSessions].some(value => value === null),
    uniqueViewers,
    uniqueAcceptors,
    uniquePassers,
    uniqueCompleters,
    uniqueMaintainerReplies,
    ...
    maintainerReplyRate: rate(replied.size, accepted.size),
    averageMinutesToMaintainerReply: replyCohorts.size >= threshold && responseMinutes.length >= threshold
      ? Math.round(responseMinutes.reduce((total, minutes) => total + minutes, 0) / responseMinutes.length)
      : null,
