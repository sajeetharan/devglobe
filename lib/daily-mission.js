...
export function missionType(opportunity) {
...
export function selectDailyMission(opportunities, { login, now = new Date(), excludedIssueIds = [] } = {}) {
  ...
  return {
    ...
    durationMinutes: opportunity.estimatedMinutes || MISSION_DURATION_MINUTES,
    status: 'offered',
...
export function prioritizeMissionOpportunity(opportunities, preferredIssueId) {
  if (!Array.isArray(opportunities) || !preferredIssueId) return opportunities;
  const index = opportunities.findIndex(opportunity => String(opportunity?.id) === String(preferredIssueId));
  if (index <= 0) return opportunities;
  return [opportunities[index], ...opportunities.slice(0, index), ...opportunities.slice(index + 1)];
}
