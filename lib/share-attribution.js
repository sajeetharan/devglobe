...
const SOCIAL_CAMPAIGNS = new Set(['country_leaderboard', 'developer_spotlight', 'identity_card', 'india_top_50', 'rank_movement', 'vibe_session']);
const SHARE_CHANNELS = new Set(['copy_link', 'facebook', 'linkedin', 'native_share', 'reddit', 'share_page', 'x']);
const ACQUISITION_SOURCES = new Set([...SOCIAL_SOURCES, 'community_event', 'direct', 'external_referral', 'github_readme', 'manual_outreach', 'other', 'weekly_digest', 'weekly_spotlight']);
...
export function acquisitionAttributionProperties(params, { referrer = '', siteUrl = '' } = {}) {
  ...
  const defaultChannel = source === 'direct' ? 'direct'
    : source === 'external_referral' || source === 'github_readme' || source === 'copy_link' || source === 'share_page' ? 'referral'
      : source === 'weekly_digest' ? 'email'
        : source === 'community_event' || source === 'manual_outreach' ? 'community'
...
export function vibeCardShareUrl(siteUrl, login, vibeId, channel) {
  const targetLogin = normalizeDeveloperLogin(login);
  const safeId = String(vibeId || '').trim().toLowerCase();
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(safeId)) {
    throw new TypeError('A valid Vibe Card ID is required');
  }
  const source = normalizeShareChannel(channel);
  const url = new URL(`/vibe/${encodeURIComponent(targetLogin)}/${encodeURIComponent(safeId)}`, siteUrl);
  url.searchParams.set('utm_source', source);
  url.searchParams.set('utm_medium', source === 'copy_link' ? 'referral' : 'social');
  url.searchParams.set('utm_campaign', 'vibe_session');
  url.searchParams.set('utm_content', targetLogin);
  return url.toString();
}
