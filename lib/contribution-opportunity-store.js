...
export async function reserveMissionPreview(container, clientHash, now = new Date()) {
  const id = `mission-preview:${clientHash}`;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    let current;
    try {
      ({ resource: current } = await container.item(id, id).read());
    ...
    const timestamps = (current?.timestamps || []).filter(timestamp => Date.parse(timestamp) > now.getTime() - PREVIEW_WINDOW_MS);
    if (timestamps.length >= MAX_PREVIEWS_PER_WINDOW) {
      return Math.max(1, Math.ceil((Date.parse(timestamps[0]) + PREVIEW_WINDOW_MS - now.getTime()) / 1000));
    }
    const next = {
      id,
      documentType: 'mission-preview-quota',
      timestamps: [...timestamps, now.toISOString()],
      ttl: Math.ceil(PREVIEW_WINDOW_MS / 1000) * 2,
    };
    try {
      if (current) {
        await container.item(id, id).replace(next, { accessCondition: { type: 'IfMatch', condition: current._etag } });
      } else {
        await container.items.create(next);
      }
      return 0;
