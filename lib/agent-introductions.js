...
export function createIntroductionDocument(input, agent, now = new Date()) {
  ...
  return {
    ...
    auditTrail: [{ status: 'pending', at: now.toISOString(), actor: 'requester-agent' }],
  };
}

export function appendIntroductionAudit(request, status, at) {
  const existing = Array.isArray(request?.auditTrail) ? request.auditTrail : [{
    status: 'pending',
    at: request.createdAt,
    actor: 'requester-agent',
  }];
  return [...existing, { status, at, actor: 'developer' }].slice(-20);
}
